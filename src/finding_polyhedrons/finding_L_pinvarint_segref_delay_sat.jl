...
function step1_saturation_segref_alt(A, B, C, S, R; f=12, lambda=0.99, pond=0.5, symetric=true, time=20)
    model = Model() do
        return NEOSServer.Optimizer(; email = "wallace.lopes.162@ufrn.edu.br", solver = "Knitro")
    end
    ...
    # --- Pré-cálculo das Matrizes de Equilíbrio L_x e L_u ---
    M_eq = [A - I(n) B; C zeros(size(C, 1), m)]
    L_mat = inv(M_eq) * [zeros(n, size(C, 1)); I(size(C, 1))]
    Lx = L_mat[1:n, :]
    Lu = L_mat[n+1:end, :]
    ...
    @constraint(model, M*F .== -F*KA)
    @constraint(model, N*F .== -F*KG)
    @constraint(model, Pref * R .== F * B * Lu - F * BG * Lx)
...
function step2_saturation_segref_alt(A, B, C, S, R, G, umax, umin, d; f=12, lambda=0.99, symetric=true, time=20)
    model = Model() do
        return NEOSServer.Optimizer(; email = "wallace.lopes.162@ufrn.edu.br", solver = "Knitro")
    end
    ...
    # Variáveis do Setor de Saturação e Invariância
    @variable(model, -100 <= K[1:n, 1:n] <= 100)
    @variable(model, -100 <= F[1:f, 1:n] <= 100)
    @variable(model, -100 <= Psat[1:m, 1:n] <= 100) 

    @variable(model, 0.001 <= igamma <= 1000)
    @variable(model, J[1:n, 1:f])

    # Multiplicadores de Farkas para os limites de saturação (Estado)
    @variable(model, 0 <= Q1[1:m, 1:f] <= 100)
    @variable(model, 0 <= Q2[1:m, 1:f] <= 100)

    # CORREÇÃO: Multiplicadores de Farkas declarados para acoplar a Referência
    @variable(model, 0 <= Qref1[1:m, 1:r] <= 100)
    @variable(model, 0 <= Qref2[1:m, 1:r] <= 100)
    ...
    if symetric
        ...
        H = Hp - Hm; L1 = L1p - L1m; L2 = L2p - L2m 
        M = Mp - Mm; N1 = N1p - N1m; N2 = N2p - N2m
        Pref = Pref1 - Pref2; T = T1 - T2; jota = jota1 - jota2
    else
        @variable(model, 0 <= H[1:f, 1:f] <= 200)
        @variable(model, 0 <= L1[1:f, 1:f] <= 200)
        @variable(model, 0 <= L2[1:f, 1:f] <= 200)
    ...
    # Restrições Bilineares Auxiliares
    @constraint(model, BG .== B*G)
    @constraint(model, BPsat .== B*Psat)
    @constraint(model, KA .== K*(A - I))
    @constraint(model, KG .== K*BG)
    ...
    # Igualdade da Referência Dinâmica
    @constraint(model, Pref * R .== F * B * Lu - F * BG * Lx)

    # Igualdades de Farkas para a Dinâmica [cite: 17, 18]
    @constraint(model, H*F .== F*A + FK)
    @constraint(model, L1*F .== F*BG - FK)
    @constraint(model, L2*F .== F*BPsat - FK)
    @constraint(model, M*F .== -F*KA)
    ...
    # --- Restrições de Igualdade de Farkas para Saturação [cite: 18] ---
    @constraint(model, Q1*F .== Psat)          
    @constraint(model, Qref1*R .== Lu - G*Lx)  

    @constraint(model, Q2*F .== -Psat)         
    @constraint(model, Qref2*R .== G*Lx - Lu)  

    # --- Restrições de Desigualdade de Limite de Atuador [cite: 18] ---
    @constraint(model, Q1*ones_f + Qref1*ones_r .<= umax)   
    @constraint(model, Q2*ones_f + Qref2*ones_r .<= -umin)  

    # Restrições de Inclusão no Estado S [cite: 11]
    @constraint(model, T*F .== S)
